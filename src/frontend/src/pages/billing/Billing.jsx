  ...
  useEffect(() => {
    ...
    eventSource.addEventListener("operator-bill-update", (event) => {
      try {
        ...
        console.log("Operator bill update received:", billData);

        if (role === "OPERATOR") {
          console.log(
            "Adding new bill to operator billing history:",
            billData.billId
          );

          // Update billing data with new bill
          setBillingData((prevData) => {
            if (!prevData) return prevData;

            const newData = { ...prevData };
            const existingBills = newData.allTripBills || [];
            const isAlreadyInList = existingBills.some(
            ...
            if (!isAlreadyInList) {
              const newBill = {
                ...
                tripId: billData.tripId,
                userId: billData.userId,
                userEmail: billData.userEmail,
                userFullName: billData.userFullName,
                bikeId: billData.bikeId,
                startStationName: billData.startStationName,
                endStationName: billData.endStationName,
                startTime: billData.startTime,
                endTime: billData.endTime,
                durationMinutes: billData.durationMinutes,
                billStatus: billData.billStatus,
                pricingStrategy: billData.pricingStrategy,
                baseFare: billData.baseFare,
                perMinuteRate: billData.perMinuteRate,
                totalAmount: billData.totalAmount,
              };

              console.log("Adding new bill to operator history:", newBill);
              newData.allTripBills = [newBill, ...existingBills];

              newData.totalSystemTrips = (newData.totalSystemTrips || 0) + 1;
              if (billData.billStatus === "PENDING") {
                newData.totalSystemOutstandingBills =
                  (newData.totalSystemOutstandingBills || 0) + 1;
                newData.totalSystemOutstandingAmount =
                  (newData.totalSystemOutstandingAmount || 0) +
                  billData.totalAmount;
              } else {
                newData.totalSystemRevenue =
                  (newData.totalSystemRevenue || 0) + billData.totalAmount;
              }
            }
        ...
        console.error("Error processing operator bill update:", error);
      }
    });

    // Error handling with auto-reconnect
    eventSource.onerror = (error) => {
      console.error("Billing SSE connection error:", error);
      try {
        eventSource.close();
      } catch (e) {
        /* ignore */
      ...
      if (sseRetryCount < MAX_RETRIES) {
        const timeout = RETRY_DELAY * Math.pow(2, sseRetryCount);
        console.log(
          `Retrying Billing SSE connection in ${timeout}ms (attempt ${
            sseRetryCount + 1
          }/${MAX_RETRIES})`
        );
        ...
        console.error("Max Billing SSE retry attempts reached");
      }
    };

    // Cleanup
    return () => {
      console.log("Cleaning up Billing SSE connection");
      try {
        eventSource.close();
      } catch (e) {
        /* ignore */
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/api/billing/user/payment",
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        alert(
          "Payment Failed\n\nThe payment could not be processed. Please verify that your card information matches the details on file:\n\n• Card number\n• Cardholder name\n• Expiry date\n• CVV\n\nError: " +
            (errorData.message || "Invalid payment information")
        );
                        ...
                        <div key={tripBill.tripId} className="trip-bill-card">
                          ...
                          <div className="card-header">
                            <div className="trip-id">
                              <i className="fas fa-route"></i> Bill #
                              {tripBill.billId}
                            </div>
                            <span
                              className={`status-badge ${tripBill.billStatus.toLowerCase()}`}
                            >
                              {tripBill.billStatus}
                            </span>
                          </div>
                          ...
                          <div className="pricing-section">
                            <div className="pricing-row">
                              ...
                              <span>
                                Rate: ${tripBill.perMinuteRate.toFixed(2)}/min
                              </span>
                            </div>
            ...
            {selectedBill && role !== "OPERATOR" ? (
                ...
                <div className="payment-details">
                  ...
                  <div className="payment-detail-item">
                    ...
                    <span className="route-text">
                      {selectedBill.startStationName} →{" "}
                      {selectedBill.endStationName}
                    </span>
                  </div>
                  <div className="payment-detail-item">
                    <span>Duration</span>
                  ...
                  <div className="form-row">
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handlePaymentFormChange("expiryMonth", e.target.value)
                        }
                        maxLength="2"
                        required
                        disabled={paymentProcessing}
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handlePaymentFormChange("expiryYear", e.target.value)
                        }
                        maxLength="2"
                        required
                        disabled={paymentProcessing}
                    ...
                    <div className="form-group">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handlePaymentFormChange("cvc", e.target.value)
                        }
                        maxLength="3"
                        required
                        disabled={paymentProcessing}
              ...
              <div className="no-reservation-card">
                <h3>
                  ...
                  {role === "OPERATOR" ? "System Summary" : "Account Summary"}
                </h3>
                {role !== "OPERATOR" && (
                ...
                {billingData && (
                  <>
                    ...
                    <p className="summary-stat">
                      ...
                      {role === "OPERATOR"
                        ? billingData.totalSystemTrips
                        : billingData.totalTrips}
                    </p>
                    {role !== "OPERATOR" && (
                      <p className="summary-stat">
                        <strong>Total Spent:</strong> $
                        {billingData.totalAmountSpent.toFixed(2)}
                      </p>
                    )}
                    {role === "OPERATOR" && (
                      <p className="summary-stat">
                        <strong>Total Revenue:</strong> $
                        {billingData.totalSystemRevenue.toFixed(2)}
                      </p>
                    )}
                    <div className="summary-divider"></div>
                    <p className="summary-stat">
                      <strong>Outstanding Bills:</strong>{" "}
                      {role === "OPERATOR"
                        ? billingData.totalSystemOutstandingBills
                        : billingData.totalOutstandingBills}
                    </p>
                    <p className="summary-stat">
                      <strong>Amount Due:</strong> $
                      {role === "OPERATOR"
                        ? billingData.totalSystemOutstandingAmount.toFixed(2)
                        : billingData.totalOutstandingAmount.toFixed(2)}
                    </p>
                  </>
                )}
              </div>
