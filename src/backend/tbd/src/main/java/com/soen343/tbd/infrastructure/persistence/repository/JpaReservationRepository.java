...
public interface JpaReservationRepository extends JpaRepository<ReservationEntity, Long> {
    ...
    // Count expired reservations  by user since a specific date
    @Query("SELECT COUNT(r) FROM ReservationEntity r WHERE r.user.userId = :userId " +
           "AND r.status = :status " +
           "AND r.reservedAt >= :since")
    int countByUserIdAndStatusSince(@Param("userId") Long userId, @Param("status") ReservationStatus status, @Param("since") LocalDateTime since);
}
